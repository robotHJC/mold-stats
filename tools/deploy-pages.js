...
console.log('[1/4] 已同步 docs/index.html（' + (src.length / 1024).toFixed(0) + ' KB）');

/* 2. git */
if (!fs.existsSync(path.join(ROOT, '.git'))) {
  run('git', ['init', '-b', 'main']);
  console.log('[2/4] git init');
} else {
  console.log('[2/4] 已经是 git 仓库');
}
run('git', ['add', '-A']);
...
/* 4. 打开 Pages */
console.log('[4/4] 启用 Pages（main / docs）…');
